...
import { updateWeeklyObjective } from "../services/weeklyObjectiveService";
...
function normalizeStartDate(date) {
    const safeDate = new Date(date || new Date());
...
function normalizeObjectiveResponsible(item) {
    if (!item) return null;
...
function getInitialSelectedUsersFromObjective(objective, currentUser) {
    if (Array.isArray(objective?.assignedUsers) && objective.assignedUsers.length > 0) {
        return objective.assignedUsers.map(normalizeObjectiveResponsible).filter(Boolean);
    }

    if (
        Array.isArray(objective?.assignedToUsers) &&
        objective.assignedToUsers.length > 0
    ) {
        return objective.assignedToUsers
            .map(normalizeObjectiveResponsible)
            .filter(Boolean);
    }

    if (Array.isArray(objective?.responsables) && objective.responsables.length > 0) {
        return objective.responsables.map(normalizeObjectiveResponsible).filter(Boolean);
    }

    if (objective?.assignedTo) {
        return [
            {
                uid: String(objective.assignedTo),
                name: objective?.assignedToName || "Usuario",
                email: "",
            },
        ];
    }
...
export default function EditWeeklyObjectiveScreen({ navigation, route }) {
    ...
    const initialStartDate = useMemo(() => {
        return normalizeStartDate(getObjectiveStartDate(objective) || new Date());
    }, [objective]);

    const initialEndDate = useMemo(() => {
        return normalizeEndDate(
            getObjectiveEndDate(objective) ||
                getObjectiveStartDate(objective) ||
                new Date()
        );
    }, [objective]);

    const initialCategory = getNoteCategoryByKey(objective?.categoryKey, isDarkMode);
    ...
    const [selectedUsers, setSelectedUsers] = useState(() =>
        getInitialSelectedUsersFromObjective(objective, user)
    );
    const [assignDialogVisible, setAssignDialogVisible] = useState(false);

    const [categoryKey, setCategoryKey] = useState(
        objective?.categoryKey || initialCategory?.key || NOTE_CATEGORIES[0]?.key
    );
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const category = getNoteCategoryByKey(categoryKey, isDarkMode);
            const firstAssignedUser = selectedUsers[0];

            await updateWeeklyObjective(objective.id, {
                title: title.trim(),
            ...
            console.log("UPDATE WEEKLY OBJECTIVE ERROR:", error);
    ...
    const currentPickerValue = activeDatePicker === "end" ? endDate : startDate;
                    ...
                    >
                        <View style={styles.headerBlock}>
                            ...
                            <Text variant="bodyMedium" style={styles.subtitle}>
                                Actualizá el título, rango de fechas, categoría o responsables del objetivo.
                            </Text>
                        </View>
                        ...
                        <View style={styles.dateSelectorsRow}>
                            ...
                            >
                                ...
                                <View style={styles.dateSelectorTextWrap}>
                                    <Text style={styles.dateSelectorLabel}>Inicio</Text>
                            ...
                            >
                                ...
                                <View style={styles.dateSelectorTextWrap}>
                                    <Text style={styles.dateSelectorLabel}>Final</Text>
                            ...
                            <Card.Content style={styles.cardContent}>
                                ...
                                <View style={styles.infoBox}>
                                    ...
                                    <Text style={styles.infoText}>
                                        Al guardar, el objetivo se actualizará en el calendario ocupando desde la fecha de inicio hasta la fecha final.
                                    </Text>
                                </View>
